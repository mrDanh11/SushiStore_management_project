...
async function fetchDishesByBranch() {
    const queryParams = new URLSearchParams({ userID: userID }).toString();
    try {
        const response = await fetch(`/employee/dish/getDishbyEmployeeID?${queryParams}`);
        if (!response.ok) {
...
document.getElementById('submitButton').addEventListener('click', async () => {
    ...
    const SobanInput = document.getElementById('Soban');
    const SobanValue = SobanInput.value.trim();
    ...
    try {
        const resMaxMP = await fetch(`/employee/reserve/getMaxMP`);
        if (!resMaxMP.ok) {
            throw new Error('Không thể lấy dữ liệu món ăn');
        }
    ...
    try {
        const response = await fetch('/employee/reserve/dishSubmit', {
            method: 'POST',
            headers: {
