...
document.getElementById('submitButton').addEventListener('click', async () => {
    ...
    const guestCount = document.getElementById('guests').value; // Số lượng khách
    const reservationTime = document.getElementById('reservation-time').value; // Thời gian đặt bàn
    const notes = document.getElementById('notes').value; // Ghi chú

    const reservationDate = new Date(reservationTime);
    const date= `${reservationDate.getFullYear()}/${reservationDate.getMonth() + 1}/${reservationDate.getDate()}`
    const time = `${reservationDate.getHours()}:${reservationDate.getMinutes()}:${reservationDate.getSeconds()}`;
    ...
    try {
        const response = await fetch('/Customer/reserve/tableSubmit', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
            body: JSON.stringify({
                branchId: document.getElementById('cn').value,
                khuvucId: document.getElementById('cn').options[document.getElementById('cn').selectedIndex].getAttribute('data-KV'),
